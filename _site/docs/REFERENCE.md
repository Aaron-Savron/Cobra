...
Parser declarations now construct and attach canonical descriptors directly, and IR inference propagates those descriptors through locals, calls, and inferred expressions. A small set of scalar compatibility fields remains in the AST and native symbol records for transitional code-generation details; they are not used to reconstruct type identity. This is an internal compiler contract, not a user-facing generics feature yet. New type features should extend the canonical descriptor instead of adding another parallel metadata field.
