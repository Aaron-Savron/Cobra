...
* `struct Name: { field: type, ... }` declares a user type. Supported field types are `i64`, `i32`, `u32`, `u64`, `f32`, `bool`, and `string`. Struct variables are zero-initialized contiguous frame regions; `var.field` reads and `var.field = value` writes lower to single native loads/stores using IR-assigned offsets. Unknown fields, duplicate fields, and type-mismatched member assignments are rejected during validation. Scalar-only structs can be passed as semantic by-value parameters: the caller supplies one pointer slot and the callee copies the bytes into private frame storage before execution. Structs may also contain explicit borrowed byte views using `readonly []u8` or `out []u8`; each view occupies a pointer-plus-length pair and keeps the owner relationship visible to validation. Readonly fields can be initialized once on a local, then cannot be rebound or written through a parameter. `out` fields permit indexed byte writes. Each field keeps its own borrow owner and region identity, so multiple request views cannot overwrite one another and a view becomes invalid when its region ends. Struct returns use the caller-owned sret path described below. Nested structs, unqualified byte views, and other ownership-bearing fields remain outside this lane.
...
Use `is_some`, `unwrap`, `is_ok`, `unwrap_ok`, and `unwrap_err` with a named value. An invalid unwrap reports a native Cobra failure and exits. Sum values are supported as locals, function returns, and scalar-only function parameters. A scalar sum parameter is passed as one pointer to caller-owned frame storage, then copied into callee-private storage before inspection. A struct payload is copied into the caller's sret buffer with a native tag followed by the payload bytes and, for `Result`, the error bytes. Scalar-only struct parameters are supported with semantic by-value copies and one pointer ABI slot. Borrowed `readonly []u8` and writable `out []u8` fields preserve their pointer-plus-length layout when the struct is copied. A callee cannot free a borrowed field, freeing the owner while a struct view is live is rejected, and returning a struct with borrowed fields is currently rejected; proving a safe owner relationship is deferred until the full lifetime model exists. Sum parameters currently require named values with scalar payloads, and nested sums, views, strings, collections, and ownership-bearing structs remain rejected until their ownership layouts are defined. Scalar generic helpers now support monomorphic `T` and borrowed `readonly []T` parameters for `[]i64`, `[]f32`, and `[]u8`; mutable generic slices, owned collections, generic slice returns, and lifetime-polymorphic values remain rejected. `?` preserves the full tagged struct result on failure and copies the successful struct payload into the caller's result storage. `?` inside an unsupported `@parallel` body causes the body to keep its checked scalar lowering rather than entering the worker path. See `examples/59_option_result.cb`, `examples/61_typed_propagation.cb`, `examples/62_sum_interactions.cb`, `examples/71_struct_sum_payloads.cb`, `examples/98_sum_parameters.cb`, and `examples/99_sum_abi_matrix.cb` for register, stack, mixed-class, module, and nested-return coverage.

### Shared composite layout

Cobra uses one native layout contract for structs. The IR computes field offsets and total size, and the direct emitter consumes those same values. Scalar fields keep their existing native slots, and qualified top-level byte views occupy a pointer plus length pair. Nested by-value structs are laid out recursively, but nested fields are scalar-only until ownership metadata is part of the layout. Fields stay in declaration order without internal padding, and the complete value is rounded to an 8-byte boundary. A by-value cycle such as `struct Node: { next: Node }` is rejected because it has no finite representation. This is a Cobra-native packed layout: fields stay in declaration order without internal padding, and the complete value is rounded to an 8-byte boundary. It is not a natural C struct layout. Do not cast a Cobra struct pointer to an ordinary C struct pointer unless an adapter uses the documented field offsets; C-compatible padding and alignment require a separate ABI contract.

Struct parameters use one pointer ABI slot and the callee copies the complete value into private frame storage. That gives mutation predictable value semantics without sharing the caller's storage. Nested field reads and scalar writes use the computed offsets directly. Borrowed slice fields still require explicit qualifiers and remain separate from the recursive scalar layout contract until ownership-bearing composites are unified.

See `examples/90_recursive_struct_layout.cb` for nested field access, private parameter copies, and the corresponding cycle diagnostic. The layout also records whether each field is scalar, borrowed, or owned. Nested borrowed and owned fields are rejected until that metadata is connected to copy, move, and cleanup analysis.
...
Parser declarations now construct and attach canonical descriptors directly, and IR inference propagates those descriptors through locals, calls, and inferred expressions. A small set of scalar compatibility fields remains in native symbol records for transitional code-generation details; they are not used to reconstruct type identity. Canonical generic instantiation currently has one deliberately narrow lane: `cobra_type_instantiate` substitutes one `GENERIC_PARAM` in scalar `Option[T]` or `Result[T, E]` descriptors, finalizes the native ABI, and interns equivalent results. Source-level functions with one scalar parameter now specialize monomorphically at call sites, for example `unwrap_or[T](value: Option[T], fallback: T) -> T`. Repeated calls reuse the canonical specialization, while unresolved, composite, mismatched, and recursive substitutions are rejected before code generation. Borrowed `readonly []T` parameters now specialize to the existing pointer-plus-length ABI for scalar elements; mutable slices, owned collections, and lifetime-polymorphic generic values remain deferred. Immutable scalar-only generic structs now specialize fields into packed `Box__i64`/`Box__f32` descriptors, support by-value parameter copies and caller-owned struct returns, and reuse canonical specializations across repeated and module-boundary calls. Generic structs may also contain `readonly []T` borrowed fields in non-escaping parameters and locals; `View[i64]`, `View[f32]`, and `View[u8]` preserve pointer-plus-length ABI, readonly mutability, and region ownership. Owned, mutable, returning, heap-stored, recursive, and lifetime-dependent generic structs are rejected before ABI lowering. See `examples/107_generic_borrowed_views.cb` and `examples/108_generic_borrowed_view_module.cb`. New type features should extend the canonical descriptor instead of adding another parallel metadata field.
