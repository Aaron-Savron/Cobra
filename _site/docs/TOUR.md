...
The compiler is also introducing one canonical recursive type descriptor behind this surface. It records type identity, generic arguments, nested payloads, field layout, ownership, mutability, region origin, and ABI class in one graph. The first source-level generic lane is now live: one scalar `T` specializes functions such as `unwrap_or[T]` at monomorphic call sites, with canonical interning and ABI validation. Composite and ownership-polymorphic generics remain deferred. Readonly borrowed slices are supported as a narrow composite lane: `readonly []T` specializes to the existing pointer-plus-length ABI for scalar `i64`, `f32`, and `u8` elements without transferring ownership. Immutable scalar-only generic structs now substitute fields into packed native layouts, support by-value parameters and caller-owned returns, and reuse local and module-boundary specializations. Generic borrowed-view structs may contain `readonly []T` fields in non-escaping parameters and locals; their i64, f32, and u8 specializations keep pointer-plus-length ABI and region ownership. Returning, heap-storing, freeing, mutating, or using them after a region ends is rejected, as are owned, recursive, and lifetime-dependent forms. Generic functions can compose with borrowed-view structs as `count[T](view: View[T]) -> i64`; call sites infer `i64`, `f32`, or `u8`, reuse specializations, and retain the same region and escape checks. See `examples/100_generic_functions.cb`, `examples/101_generic_module.cb`, `examples/103_generic_readonly_slices.cb`, `examples/104_generic_readonly_slice_module.cb`, `examples/105_generic_structs.cb`, `examples/106_generic_struct_module.cb`, `examples/116_generic_view_functions.cb`, and `examples/117_generic_view_function_module.cb`.
