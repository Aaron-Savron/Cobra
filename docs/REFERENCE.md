...
Lists also expose the everyday Python operations, in both direct (`sort(xs)`) and method (`xs.sort()`) forms. `sort`, `reverse`, and `clear` mutate in place; `count(xs, v)` counts occurrences; `index(xs, v)` returns the first position or -1 when absent; `extend(xs, ys)` appends every element of another list; `insert(xs, i, v)` shifts elements right; and `remove(xs, v)` drops the first occurrence and returns 1, or 0 when absent (the collections runtime prefers testable sentinels to Python's `ValueError`). `xs[a:b]` slices into a fresh owned copy with Python bounds semantics (negative bounds count from the end, out-of-range clamps, an empty window when `start >= end`); string slicing keeps its `substring` semantics. Indexing accepts negative positions on both read and write: `xs[-1]` is the last element, and out-of-range negative indices still fail the bounds check. `==` and `!=` on lists compare contents rather than descriptor pointers: lengths must match and every element must compare equal, with string elements compared by value. These operations support the integer family, `f32`, and `string` element types; struct-element lists are rejected until their comparison and destruction contracts are defined. See `examples/176_list_methods.cb`. List comprehensions build a fresh native list in one pass: `[expr for value in values]` and `[expr for value in values if guard]` lower to a single loop with one `cobra_list_append_*` call per kept element, writing directly into the declared `list` symbol. Local lists and dictionaries are reclaimed automatically when their containing function exits, including early returns. `free` remains available as an explicit compatibility operation for owned values; borrowed collection parameters cannot be freed, use-after-free is rejected, and unsafe owned aliases are rejected. Dictionary keys are copied into the table. There is no tracing garbage collector, boxing, or hidden list object on fixed-array/tensor paths. `list[i64]` and `list[f32]` are the first native element types; dictionary values are currently `i64`, and missing `get` keys return the supplied fallback.
...
* `struct Name: { field: type, ... }` declares a user type. Supported field types include scalar values, borrowed byte views, owned strings, owned slices, and supported owning `Option`/`Result` payloads. Struct variables are zero-initialized contiguous frame regions; `var.field` reads and `var.field = value` writes use canonical field offsets. Unknown fields, duplicate fields, and type-mismatched member assignments are rejected during validation. Struct parameters use one pointer ABI slot and are copied or moved according to field ownership. Borrowed views preserve their pointer-plus-length, owner, and region metadata. Owned fields use explicit move, replacement, return, and drop operations. Struct returns use the caller-owned sret path described below. The backend-IR pipeline (`--backend=native`) supports these ownership-bearing fields through its evaluator and Linux x86-64 emitters; generic ownership-bearing fields and arbitrary non-scalar nesting remain deferred.

See `examples/50_type_foundation.cb`.

Function parameters require an explicit type annotation (`def f(x: i64): { ... }`);
an omitted parameter type is a compile-time error rather than a silent default.
Function bodies may use the original brace form or Python-style indentation
(`def f(x: i64) -> i64:` followed by an indented body). `elif`, `and`, `or`,
`True`, `False`, and `None` are accepted spellings; `and`/`or` lower to eager
scalar boolean expressions, so use an explicit `if` when short-circuiting side
effects matters. `let` locals remain inferrable from their
initializer (`let x = 5`). Function return types may still be omitted, defaulting
to `i64` if a value is returned or `void` otherwise; this is an established,
widely-used convention, not the same class of bug.
...
Use `is_some`, `unwrap`, `is_ok`, `unwrap_ok`, and `unwrap_err` with a named value. An invalid unwrap reports a native Cobra failure and exits. Sum values are supported as locals, function returns, and parameters. Scalar and supported ownership-bearing sums use explicit tagged storage, caller-provided aggregate returns, and field-aware move and drop operations. Borrowed fields cannot be freed by a callee, and freeing an owner while a view is live is rejected. In the backend-IR pipeline (`--backend=native`), direct owned string and owned slice payloads, nested owning sums, indirect calls, returns, extraction, and recursive destruction are supported. Payload-carrying enums, arbitrary non-scalar payloads, and ownership-bearing generic sums remain deferred. Scalar generic helpers support monomorphic `T`, borrowed `readonly []T` parameters and returns, and writable `out []T` parameters and returns for scalar elements. Calls preserve allocation and region provenance, while local, region, unresolved recursive, and owner-preserving generic returns remain rejected. `?` preserves the full tagged result on failure and copies successful payloads into caller storage. See `examples/59_option_result.cb`, `examples/61_typed_propagation.cb`, `examples/62_sum_interactions.cb`, `examples/71_struct_sum_payloads.cb`, `examples/98_sum_parameters.cb`, and `examples/99_sum_abi_matrix.cb` for register, stack, mixed-class, module, and nested-return coverage.
...
Strings are immutable NUL-terminated byte strings represented by one native pointer. Literals and typed `string` parameters/returns use the ordinary GPR ABI without a hidden object header. `+` and `concat(a, b)` allocate a new owned string; `f"..."` format literals lower to `str()` conversions joined with `+` (so `f"n = {n}"` builds an owned string); comparisons are lexicographic; `starts_with`, `ends_with`, and `contains` return integer booleans; `char_at` returns a byte and traps on invalid indexes. Release concatenated or functions proven to return fresh concatenation storage with `string_free`; literals and borrowed forwarded values are not owned. `string_from_bytes(buf, len)` allocates and copies `len` bytes out of a `[]u8` buffer plus a NUL terminator, producing an owned string usable anywhere a string is (including as a `dict[string]...` key) from runtime-derived data such as parsed input or file contents, not just source literals and concatenation. Source escapes decode `\\n`, `\\r`, `\\t`, `\\\\`, and `\\\"`.
